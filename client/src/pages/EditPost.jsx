...
import { PenTool, Image, Tag, Folder, Upload, Loader, ArrowLeft } from 'lucide-react';
...
const EditPost = () => {
    ...
    const fetchPost = async () => {
      try {
        ...
        if (response.data.success) {
          ...
          setPrice(post.price !== undefined ? post.price.toString() : '0.99');
        }
      } catch (err) {
        console.error('Error fetching post for edit:', err);
        setError('Failed to load article details. It may not exist.');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="max-w-3xl mx-auto py-10 px-4 md:px-0 relative">
      <Link to={`/post/${id}`} className="inline-flex items-center space-x-2 text-xs font-semibold text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white mb-6 transition-colors">
      ...
      <div className="flex items-center space-x-3 mb-8">
        ...
        <h1 className="text-3xl font-extrabold text-slate-800 dark:text-slate-100 tracking-tight">Edit Article</h1>
      </div>
          ...
          <div>
            ...
            <div className="grid md:grid-cols-3 gap-4 items-center">
              ...
              <div className="h-12 border border-slate-200 dark:border-slate-800 rounded-xl bg-slate-50 dark:bg-slate-950 flex items-center justify-center text-xs text-slate-400 overflow-hidden relative shadow-inner">
                {uploading ? (
                  <div className="flex items-center space-x-1 text-indigo-650 dark:text-indigo-400">
